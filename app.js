...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // -------------------------------------------------------------
    // 2. STATE MANAGEMENT (localStorage + Firebase Sync)
    // -------------------------------------------------------------
    ...
    const firestoreAvailable = !!window.db;
    const firestoreRoot = firestoreAvailable ? window.db.collection('estudioflow') : null;
    let isApplyingRemoteUpdate = false;

    const persistFirestoreDoc = (docId, payload) => {
        if (!firestoreAvailable || isApplyingRemoteUpdate) return;
        firestoreRoot.doc(docId).set(payload, { merge: true }).catch((err) => {
            console.warn(`Firestore sync failed for ${docId}:`, err);
        });
    };

    const syncTasks = () => persistFirestoreDoc('tasks', { tasks });
    const syncRoutines = () => persistFirestoreDoc('routines', { morningRoutines, afternoonRoutines, eveningRoutines });
    const syncWater = () => persistFirestoreDoc('water', { waterCount, waterTarget });
    const syncNotebooks = () => persistFirestoreDoc('notebooks', { notebooks, activeNotebookId });
    const syncPunishments = () => persistFirestoreDoc('punishments', { punishmentsList });
    ...
    const saveState = (key, value) => {
        ...
        switch (key) {
            ...
            case 'routines-afternoon':
            case 'routines-evening':
                syncRoutines();
                break;
            case 'studyflow-water-count':
            case 'studyflow-water-target':
                syncWater();
                break;
            case 'studyflow-notebooks-list':
            case 'studyflow-active-notebook-id':
                syncNotebooks();
                break;
            case 'routines-punishments-list':
                syncPunishments();
                break;
            default:
                break;
    ...
    const attemptLogin = (email, password) => {
        if (!auth) {
            setAuthError('Firebase Auth no está disponible. Verifica la carga del script firebase-auth-compat.js.');
            return;
        }
    ...
    if (authButton) {
        authButton.addEventListener('click', () => {
            attemptLogin(authUsernameInput.value, authPasswordInput.value);
        });
    }
    ...
    if (!auth) {
        setAuthError('Firebase Auth no está disponible. Revisa la carga del script.');
        showAuthOverlay();
    } else {
        showAuthOverlay();
    }
                ...
                btnDelete.addEventListener('click', (e) => {
                    ...
                    if (confirm(`¿Estás seguro de que quieres borrar el cuaderno "${nb.title}"? Todos los apuntes en él se perderán de forma permanente. ☁️🎀`)) {
                        ...
                        if (activeNotebookId === nb.id) {
                            ...
                            saveState('studyflow-active-notebook-id', activeNotebookId);
                        }
    ...
    if (btnCreateNotebook) {
        btnCreateNotebook.addEventListener('click', (e) => {
            ...
            saveState('studyflow-active-notebook-id', newId);
    ...
    const subscribeFirestoreDoc = (docId, callback) => {
        if (!firestoreAvailable) return;
        firestoreRoot.doc(docId).onSnapshot((snapshot) => {
            if (!snapshot.exists) return;
            isApplyingRemoteUpdate = true;
            callback(snapshot.data());
            isApplyingRemoteUpdate = false;
        }, (err) => {
            console.warn(`Firestore listener error for ${docId}:`, err);
        });
    };
    ...
    const subscribeToFirestore = () => {
        ...
        subscribeFirestoreDoc('tasks', (data) => {
        ...
        subscribeFirestoreDoc('routines', (data) => {
            if (data.morningRoutines && data.afternoonRoutines && data.eveningRoutines) {
        ...
        subscribeFirestoreDoc('water', (data) => {
            if (typeof data.waterCount === 'number' && typeof data.waterTarget === 'number') {
        ...
        subscribeFirestoreDoc('notebooks', (data) => {
            if (Array.isArray(data.notebooks)) {
                notebooks = data.notebooks;
                if (data.activeNotebookId) activeNotebookId = data.activeNotebookId;
                saveState('studyflow-notebooks-list', notebooks);
                saveState('studyflow-active-notebook-id', activeNotebookId);
                renderNotebooksSidebar();
                loadActiveNotebook();
        ...
        subscribeFirestoreDoc('punishments', (data) => {
            if (Array.isArray(data.punishmentsList)) {
                punishmentsList = data.punishmentsList;
                saveState('routines-punishments-list', punishmentsList);
                renderPunishmentLedger();
                renderRouletteWheel();
                updatePunishmentWidgetState(getExpiredRoutinesCount());
            }
        });

        addTerminalLine('system', '🎉 Firebase conectado: sincronización en vivo activa.');
    };
